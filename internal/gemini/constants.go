...
const systemPrompt = "You are a YouTube video watcher, your goal is to analyze and provide nuanced responses based on the provided video" +
	...
	"\n- You must adapt the number 'evidence_timestamps' field based on how detailed the analysis needs to be or based on user's request, rather than always keeping it concise. For instance if it asked to list of exhaustive segments from the compilation video then it should be exhaustive. For simple tasks like quick video overviews, classification, or summaries, keep it short and simple." +
